...
def _make_request(image_path: str, animal: str, disease_type: str) -> dict:
    """Synchronous helper for making pet image analysis HTTP request"""
...
async def _analyze_pet_image_impl(image_path: str, animal: str, disease_type: str) -> dict:
    """
    Internal implementation of pet image analysis.
    Executes request in a thread pool to avoid blocking the event loop.
    """
    try:
        return await asyncio.to_thread(_make_request, image_path, animal, disease_type)
...
@tool
async def analyze_pet_image(image_path: str, animal: str, disease_type: str) -> dict:
    """
    ...
    """
    return await _analyze_pet_image_impl(image_path, animal, disease_type)


async def _analyze_medical_document_vlm_impl(image_path: str) -> dict:
    """
    Internal implementation of medical document analysis using VLM.
    This function is async and awaits the async vlm method.
    """
    try:
        logger.info(f"VLM Tool: Analyzing medical document: {image_path}")
        result = await analyze_medical_document_vlm(image_path)
...
@tool
async def analyze_medical_document(image_path: str) -> dict:
    """
    ...
    """
    return await _analyze_medical_document_vlm_impl(image_path)
